...
import { Palette, Eye, Download, X } from 'lucide-react';
            ...
            <tbody>
              {customOrders.map((order) => (
                <tr key={order.id} className="border-b border-vybe-glassBorder/50 hover:bg-vybe-glass/50 transition-colors">
                  ...
                  <td className="px-6 py-4">{order.baseProduct}</td>
                  <td className="px-6 py-4">{order.printLocation}</td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="flex flex-col justify-between space-y-6">
                <div className="space-y-4 text-sm">
                    ...
                    <div className="pt-4 border-t border-vybe-glassBorder">
                      ...
                      <div className="space-y-2">
                        {selectedOrder.itemsList.map((item, idx) => (
                          <div key={idx} className="bg-vybe-dark p-3 rounded-lg border border-vybe-glassBorder text-xs">
                            <div className="flex justify-between mb-1">
                              <span className="text-white font-medium">{item.name}</span>
                              <span className="text-white font-bold">₹{item.itemTotal?.toLocaleString()}</span>
                            </div>
                            <div className="text-gray-400 flex justify-between">
                              <span>Qty: {item.qty} | Size: {item.sizeName}</span>
                              <span>Base: ₹{item.price} + Print: ₹{item.customPrintPrice}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="flex justify-between border-b border-vybe-glassBorder pb-2 items-center mt-4">
                    <span className="text-gray-400 font-bold">Update Status</span>
                    <select
                      value={selectedOrder.status}
                      onChange={(e) => updateOrderStatus(selectedOrder.id, e.target.value)}
                      className="bg-vybe-dark border border-vybe-glassBorder rounded px-3 py-1.5 text-sm font-bold text-vybe-neon focus:outline-none focus:border-vybe-neon"
                    >
